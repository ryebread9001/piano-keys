...
function sendScore(score) {
	...
	xhr.send(`username=${username}&score=${score}&date=${new Date()}`);
}
...
function createTable(scores) {
	...
	tr.appendChild(dateh);
	table.append(tr);
	for (const item of scores) {
		let ob = JSON.parse(item['one'])
		let tr = document.createElement("tr");
		let td1 = document.createElement("td");
		td1.innerText = ob['username'];
		let td2 = document.createElement("td");
		td2.innerText = ob['score'];
		let td3 = document.createElement("td");
		td3.innerText = ob['date'];
		tr.appendChild(td1);
		tr.appendChild(td2);
		tr.appendChild(td3);
